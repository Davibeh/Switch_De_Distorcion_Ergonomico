...
/***********************************************
 * Function Name: AudioEffect_Select
 * Description: This function when is called will switch between sound effects
 * Must be called from the button action.
 ***********************************************/
void AudioEffect_Select(void){
		if(AudioEffect_Selection == e_ECHO){
			/*We are in Echo change to distortion*/
			AudioEffect_Selection = e_DISTORTION;
		}else{
			if(AudioEffect_Selection == e_DISTORTION){
				/*We are in distorition change to Vibrato*/
				AudioEffect_Selection = e_VIBRATO;
			}else{
				/*We are in Vibrato change to Echo*/
					AudioEffect_Selection = e_ECHO;
			}
		}
}
